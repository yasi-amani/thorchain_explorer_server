...
async function dashboardPlots() {
  ...
  await wait(2000);
  const oOne = await oldPeriodFunc(
    swaps.intervals[len - 2],
    swaps.intervals[len - 1]
  );
  await wait(2000);
  const oTwo = await oldPeriodFunc(
    swaps.intervals[len - 3],
    swaps.intervals[len - 1]
  );
  await wait(2000);
  const oThree = await oldPeriodFunc(
    swaps.intervals[len - 4],
    swaps.intervals[len - 1]
  );
  await wait(2000);
  const oldPeriodVolume = (oOne + oTwo + oThree) / 3;
  const oldTotalAverage =
    swaps.intervals.slice(-4, -1).reduce((a, c) => a + +c.totalVolumeUSD, 0) /
    3;
  const EODVolume =
    (swaps.intervals[len - 1].totalVolumeUSD * oldPeriodVolume) /
    (oldTotalAverage - oldPeriodVolume);
  ...
  const oldTVLFunc = async (start, end) => {
    ...
    return {
      oldBondEarnings: +oldBondEarnings,
      oldLiquidityEarnings: +oldLiquidityEarnings,
    };
  };

  await wait(2000);
  const tOne = await oldTVLFunc(
    earning.intervals[len - 2],
    earning.intervals[len - 1]
  );
  await wait(2000);
  const tTwo = await oldTVLFunc(
    earning.intervals[len - 3],
    earning.intervals[len - 1]
  );
  const oldPeriodBondEarning =
    (tOne.oldBondEarnings + tTwo.oldBondEarnings) / 2;
  const oldTotalBondEarnings =
    earning.intervals
      .slice(-3, -1)
      .reduce((a, c) => a + +c.bondingEarnings, 0) / 2;
  const EODBondEarnings =
    (earning.intervals[len - 1].bondingEarnings * oldPeriodBondEarning) /
    (oldTotalBondEarnings - oldPeriodBondEarning);
  const oldPeriodLiquidityEarning =
    (tOne.oldLiquidityEarnings + tTwo.oldLiquidityEarnings) / 2;
  const oldTotalLiquidityEarnings =
    earning.intervals
      .slice(-3, -1)
      .reduce((a, c) => a + +c.liquidityEarnings, 0) / 2;
  const EODLiquidityEarnings =
    (earning.intervals[len - 1].liquidityEarnings * oldPeriodLiquidityEarning) /
    (oldTotalLiquidityEarnings - oldPeriodLiquidityEarning);
  earning.intervals[len - 1].EODBondEarnings = Math.floor(EODBondEarnings);
  earning.intervals[len - 1].EODLiquidityEarnings =
    Math.floor(EODLiquidityEarnings);
...
async function nodesInfo() {
  ...
  const nodesIPUrl = `http://0.0.0.0:${process.env.PORT}${
    process.env.NETWORK === "stagenet" ? "/stage" : ""
  }/api/extraNodesInfo`;
  const chainsHeightUrl = `http://0.0.0.0:${process.env.PORT}${
    process.env.NETWORK === "stagenet" ? "/stage" : ""
  }/api/chainsHeight`;
  ...
  const maxObserevedStandby = getNodesMaxHeightOnChains(
    nodes.filter((n) => n.status === "Standby")
  );

  const { CHURNINTERVAL, HALTCHURNING, MINIMUMBONDINRUNE } = (await getMimir())
    .data;
  ...
  const ratioReward =
    (CHURNINTERVAL - (+nextChurnHeight - heights.THOR)) / CHURNINTERVAL;

  let scannerEndpoints = nodes
    .filter((n) => +n.total_bond > 300000 * 1e8)
    .map((n) => `http://${n.ip_address}:6040/status/scanner`);

  let scannerStatus = [];
  await Promise.allSettled(
    scannerEndpoints.map((promise) => axios.get(promise))
  )
    .then((res) => {
      const fulfilled = res
  ...
  const ret = nodes.map((n) => {
    ...
    // APY
    const APY =
      ((n.current_award / ratioReward) * churnsInYear) / n.total_bond ?? null;
    // Vault Membership
    ...
    if (scannerStatus[n.ip_address]) {
      for (const [chain, value] of Object.entries(
        scannerStatus[n.ip_address]
      )) {
...
async function mimirVotes() {
  try {
    const { data: mimirVotesData } = await getMimirVotes();
    const { data: mimirsData } = await getMimir();
    const { data: nodesData } = await getNodes();

    function formatVotes(mimirs) {
      const votes = {};
      mimirs.forEach((mimir) => {
        if (!mimir.value) mimir.value = 0;
        const filteredMimirs = ["KILLSWITCHSTART"];
        if (filteredMimirs.includes(mimir.key)) return;
        if (!votes[mimir.key]) {
          votes[mimir.key] = [{ signer: mimir.signer, value: mimir.value }];
        } else {
          votes[mimir.key].push({ signer: mimir.signer, value: mimir.value });
        }
      });
      return votes;
    }

    function getVoteHighestBid(voters, nodes) {
      ...
      const voteCount = values.reduce((acc, value) => {
      ...
      const votesObj = Object.keys(voteCount).map((v) => {
        ...
        const consensus = count / nodes.length;
        return {
          value: v,
          count,
          votesNeeded: Math.ceil(nodes.length * 0.66) - count,
          consensus,
        };
      });

      const hVote = votesObj.sort((a, b) => b.consensus - a.consensus)[0];
      return {
        consensus: hVote?.consensus || 0,
        votePassed: activeVoters.length,
        value: hVote?.value || "-",
        values: votesObj,
      };
    }

    const mimirVotes = formatVotes(mimirVotesData?.mimirs);
    const mimirs = { ...mimirsData, SYSTEMINCOMEBURNRATEBPS: "0" };
    const nodes = nodesData;

    if (!mimirVotes || !mimirs || !nodes) {
      console.error("Failed to fetch one or more data sets.");
      return null;
    }

    const currentVoting = [];

    for (const m of Object.keys(mimirs)) {
      if (Object.keys(mimirVotes).includes(m)) {
        const filteredVotes = [
          "ADR012",
          "ADR18",
          "ADR013",
        ...
        const hVotes = getVoteHighestBid(mimirVotes[m], nodes);
        if (hVotes.values.length === 0) continue;

        currentVoting.push({
          vote: m,
          currentVal: mimirs[m] === -1 ? "-" : mimirs[m],
          ...
          remainingVotes:
            ...
            hVotes.votePassed,
          result: +mimirs[m] === +hVotes.value ? "Passed" : "In Progress",
...
async function getNetworkAllocation() {
  ...
  for (const [address, name] of Object.entries(CEX_ADDRESSES)) {
    const balance =
      +(await getBalance(address)).find((b) => b.denom === "rune")?.amount || 0;
